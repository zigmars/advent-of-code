...
struct Range{
    Range(const int begin, const int end) : b(begin), e(end), id(-1) {}
    Range(const int begin, const int end, const int id) : b(begin), e(end), id(id) {}
    int b, e;
...
void mergeRanges(){
    ...
    for(const auto& r : range){
        if(r.b <= prev.e+1){
            if(r.e > prev.e){
                totalRange[totalRange.size()-1].e = r.e;
            }
        } else {
            totalRange.push_back(r);
        }
        prev = r;
    }
}

void printRange(vector<Range> range){
    for(auto& r : range){
        cout << "(" << r.b << "," << r.e << ") ";
    }
    cout << endl;
...
int main(){
    ...
    while(getline(cin, line) && line != ""){
        ...
        if ( regex_search(line, m, re) ){
            ...
            //cout << "(" << b1 << " : " << e1 <<"), ";
            //cout << "(" << b2 << " : " << e2 <<")" << endl;
            range.push_back(Range(b1, e1, rangeId));
            range.push_back(Range(b2, e2, rangeId));
            drange.push_back({Range(b1, e1), Range(b2, e2)});
